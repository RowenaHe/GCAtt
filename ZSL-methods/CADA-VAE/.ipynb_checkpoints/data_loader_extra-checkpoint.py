...
class DATA_LOADER(object):
    def __init__(self, dataset, aux_datasource, extra_class, extra_num_per_class, filter_lower_boundary, device='cuda', class_embedding='att'):
    ...
    # QZHe version
        # QZHe version
    def read_h5dataset(self):
        # read image feature
        
        # read image feature
        fid = h5py.File(self.datadir + 'res101.hdf5', 'r')
        ...
        fid.close()
        # read attributes
        f = open('/cver/qzhe/PyCharmOnSSH/WNids/{}/{}_{}_emb.pkl'.format(self.class_embedding,self.dataset, self.class_embedding),'rb')
        a = pickle.load(f)
        f.close()
        f =open(os.path.join(self.data_path,'extra',self.dataset,'{}_{}_emb.pkl'.format(self.dataset,self.class_embedding)),'rb')
        b = pickle.load(f)
        f.close()
        ...
        extra_feature,extra_label = choose_extra_samples(feature, label, self.extra_class, self.extra_num_per_class, self.filter_lower_boundary)
