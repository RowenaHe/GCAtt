...
class Model(nn.Module):
    ...
    def __init__(self,hyperparameters):
        ...
        self.dataset = dataloader( self.DATASET, copy.deepcopy(self.auxiliary_data_source) , self.extra_class, self.extra_num_per_class, 
        ...
        for epoch in range(0, self.nepoch):
            ...
            for iters in range(0, self.dataset.ntrain_extra, self.batch_size):
                ...
                if i%50==0 and i>0:
                    losses.append(loss)
            if self.generalized:
                unseen, seen, H = self.train_classifier()
                #print('H:',H,type(H))
    ...
    def train_classifier(self, show_plots=False):
        ...
        for k in range(self.cls_train_epochs):
            ...
            if self.generalized:
                ...
                if cls.H >= best_H:
                    ...
                    best_H = cls.H
                #print('BEST H = ',best_H)
                #history.append([torch.tensor(cls.acc_seen).item(), torch.tensor(cls.acc_unseen).item(),torch.tensor(cls.H).item()])
            else:
        ...
        '''
        if self.generalized:
            return torch.tensor(cls.acc_seen).item(), torch.tensor(cls.acc_unseen).item(), torch.tensor(
                cls.H).item(), history
        else:
